...
if __name__=="__main__":
	...
	input_im = rn.r #This is the input image
	...
	diff_im = rn - input_im
	idx = -2 
	plt.imshow(input_im, cmap = cm.Greys_r)
	plt.savefig('tmp/light_recover_'+str(idx)+'.png')
	idx += 1
	plt.imshow(rn, cmap = cm.Greys_r)
	plt.savefig('tmp/light_recover_'+str(idx)+'.png')
	idx += 1
	...
	def cb(_):
		global idx
		plt.imshow(np.abs(diff_im), cmap=cm.Greys_r)
		plt.savefig('tmp/light_recover_'+str(idx)+'.png')
		idx = idx + 1
	ch.minimize({'diff':diff_im},x0=free_variables,callback = cb)
